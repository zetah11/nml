    ...
    pub(super) fn rewrite(
        ...
        match row {
            Row::Empty => {
                ...
                (alloc.ty(Type::Invalid(e)), alloc.row(Row::Invalid(e)))
            }
            ...
            Row::Extend(old, field, rest @ Row::Var(alpha, _)) => {
                ...
                let rhs = alloc.row(Row::Extend(*label, t, r));
                self.unify_row(reporting, alloc, rest, rhs);

                let rest = alloc.row(Row::Extend(*old, field, r));
                (t, rest)
            }

            Row::Extend(old, field, rest) => {
                let (label_ty, rest) = self.rewrite(reporting, alloc, label, rest, tail);
                let rest = alloc.row(Row::Extend(*old, field, rest));
                (label_ty, rest)
            }

            Row::Invalid(e) => (alloc.ty(Type::Invalid(*e)), row),
