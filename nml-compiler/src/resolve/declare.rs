    ...
    fn declare_expr(&mut self, expr: &parsed::Expr) -> &'a declared::Expr<'a> {
        ...
        let node = match &expr.node {
            ...
            parsed::ExprNode::Record(fields, extend) => {
                ...
                let extend = extend.as_ref().map(|expr| self.declare_expr(expr));
        ...
        self.alloc.alloc(declared::Expr { node, span })
    }

    fn declare_pattern(&mut self, pattern: &parsed::Pattern) -> &'a declared::Pattern<'a> {
